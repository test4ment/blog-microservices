...
from app import methods, schemas, security, models
...
def register_user(user: schemas.UserCreate, db: Session = Depends(get_db)):
    db_user = methods.get_user_by_username(db, username=user.username)
    if db_user:
        raise HTTPException(status_code=400, detail="Username already registered")
    created_user = methods.create_user(db, user=user)
...
def login_for_access_token(db: Session = Depends(get_db), form_data: OAuth2PasswordRequestForm = Depends()):
    user = methods.get_user_by_username(db, username=form_data.username)
